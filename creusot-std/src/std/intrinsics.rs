use crate::{prelude::*, std::ptr::PtrLive};

/// [`<*const T>::add`](https://doc.rust-lang.org/core/primitive.pointer.html#method.add) with [`PtrLive`] permissions.
///
/// The four functions
///
...
/// with ghost permission tokens (`PtrLive`) that allow proving the safety conditions of that intrinsic.
///
...
#[requires(offset@ == 0 || live.contains(dst) && live.contains(dst.offset_logic(offset@)))]
...
/// [`<*const T>::offset`](https://doc.rust-lang.org/core/primitive.pointer.html#method.offset) with `PtrLive` permissions.
///
...
/// [`<*mut T>::add`](https://doc.rust-lang.org/core/primitive.pointer.html#method.add-1) with `PtrLive` permissions.
///
...
/// [`<*mut T>::offset`](https://doc.rust-lang.org/core/primitive.pointer.html#method.offset-1) with `PtrLive` permissions.
///
