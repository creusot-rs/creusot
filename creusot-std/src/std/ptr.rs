    ...
    fn invariant(self) -> bool {
        pearlite! {
            !self.ward().is_null_logic()
                && metadata_matches(*self.val_unsized(), metadata_logic(*self.ward()))
                && inv(self.val_unsized())
        }
    }
}
...
impl<T: ?Sized> Perm<*const T> {
    ...
    #[ensures(*result == *own.val_unsized())]
    #[allow(unused_variables)]
    ...
    #[ensures(*(^own).val_unsized() == ^result)]
...
impl<T> Perm<*const [T]> {
    ...
    #[ensures((^result.0.inner).val_unsized()@.len() == index)]
    #[ensures((^self).val_unsized()@ == (^result.0.inner).val_unsized()@.concat((^result.1.inner).val_unsized()@))]
    ...
    #[ensures(result.ward() == self.ward().thin())]
    #[ensures(result.len()@ == self.len())]
    pub fn live_mut<'a, 'b>(self: &'b &'a mut Self) -> PtrLive<'a, T> {
        panic!("called ghost function in normal code")
    }
}
