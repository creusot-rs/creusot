...
impl<T: Protocol> AtomicInvariantSC<T> {
    ...
    #[requires(|mode| forall<t: &mut T> t.protocol(self.public()) && inv(t) ==>
        f.precondition(mode, (t,)) &&
        // f must restore the invariant
        (forall<res: A> f.postcondition_once(mode, (t,), res) ==> (^t).protocol(self.public())))]
    #[ensures(|result, mode| exists<t: &mut T> inv(t) && t.protocol(self.public()) && f.postcondition_once(mode, (t,), result))]
...
impl<'a, T: Protocol> NonAtomicInvariantExt<'a> for Ghost<&'a NonAtomicInvariant<T>> {
    ...
    #[requires(tokens.contains(self.namespace()))]
    #[requires(|mode| forall<t: Ghost<&mut T>> (**t).protocol(self.public()) && inv(t) ==>
        f.precondition(mode, (t,)) &&
        // f must restore the invariant
        (forall<res: A> f.postcondition_once(mode, (t,), res) ==> (^t).protocol(self.public())))]
    #[ensures(|result, mode| exists<t: Ghost<&mut T>> t.protocol(self.public()) && f.postcondition_once(mode, (t,), result))]
    fn open<A, F>(self, tokens: Ghost<Tokens<'a>>, f: F) -> A
    where
        F: FnOnce(Ghost<&'a mut Self::Inner>) -> A,
    {
    ...
    Ghost<&'a L>: NonAtomicInvariantExt<'a>,
{
    ...
    #[requires(|mode| <Ghost<&'a L> as NonAtomicInvariantExt<'a>>::open.precondition(mode, (Ghost::new_logic(&**self), tokens, f)))]
    #[ensures(|result, mode| <Ghost<&'a L> as NonAtomicInvariantExt<'a>>::open.postcondition(mode, (Ghost::new_logic(&**self), tokens, f), result))]
    fn open<A, F>(self, tokens: Ghost<Tokens<'a>>, f: F) -> A
    where
        F: FnOnce(Ghost<&'a mut Self::Inner>) -> A,
    {
...
impl<T: Protocol> NonAtomicInvariant<T> {
    ...
    #[requires(|mode| forall<t: Ghost<&mut T>> t.protocol(this.public()) && inv(t) ==>
        f.precondition(mode, (t,)) &&
        // f must restore the invariant
        (forall<res: A> f.postcondition_once(mode, (t,), res) ==> (^t).protocol(this.public())))]
    #[ensures(|result, mode| exists<t: Ghost<&mut T>> inv(t) && t.protocol(this.public()) && f.postcondition_once(mode, (t,), result))]
    pub fn open<'a, A>(
        this: Ghost<&'a Self>,
