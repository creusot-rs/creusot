...
impl<T: Copy> PredCell<T> {
    ...
    #[trusted]
    #[requires(|mode| forall<x: T> self@[x] ==> f.precondition((x,), mode))]
    #[requires(|mode| forall<x: T, res: T> self@[x] && f.postcondition_once((x,), res, mode) ==> self@[res])]
    #[ensures(|_, mode| exists<x: T, res: T> self@[x] && f.postcondition_once((x,), res, mode))]
...
impl<T: ?Sized> PredCell<T> {
    ...
    pub const fn from_mut(t: &mut T, _pred: Snapshot<Mapping<T, bool>>) -> &PredCell<T> {
        unsafe { core::mem::transmute(core::cell::Cell::from_mut(t)) }
    }
...
impl<T: Default> PredCell<T> {
    ...
    #[trusted]
    #[requires(|mode| forall<x: T> T::default.postcondition((), x, mode) ==> self@[x])]
    #[ensures(self@[result])]
    pub fn take(&self) -> T {
        self.0.take()
    }
...
impl<T> PredCell<[T]> {
    ...
    pub const fn as_slice_of_cells(
        ...
        unsafe { core::mem::transmute(self.0.as_slice_of_cells()) }
    }
}
