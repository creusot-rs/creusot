...
impl<T: Copy> PredCell<T> {
    ...
    #[trusted]
    #[requires(forall<x: T> self@[x] ==> f.precondition((x,), mode!()))]
    #[requires(forall<x: T, res: T> self@[x] && f.postcondition_once((x,), res, mode!()) ==> self@[res])]
    #[ensures(exists<x: T, res: T> self@[x] && f.postcondition_once((x,), res, mode!()))]
...
impl<T: Default> PredCell<T> {
    ...
    #[trusted]
    #[requires(forall<x: T> T::default.postcondition((), x, mode!()) ==> self@[x])]
    #[ensures(self@[result])]
    pub fn take(&self) -> T {
        self.0.take()
    }
