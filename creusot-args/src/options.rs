...
pub enum RefinesCheck {
    Warn,
    NoWarn,
    Require,
}

impl ::std::fmt::Display for RefinesCheck {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            RefinesCheck::Warn => write!(f, "warn"),
            RefinesCheck::NoWarn => write!(f, "no-warn"),
            RefinesCheck::Require => write!(f, "require"),
        }
    }
}
