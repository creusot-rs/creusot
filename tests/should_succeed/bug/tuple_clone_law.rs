...
// Flat tuple, two *different* primitive leaves (each cloned per its own law).
#[ensures(result.0 == x.0 && result.1 == x.1)]
pub fn clone_flat(x: (u32, bool)) -> (u32, bool) {
    x.clone()
}

// Nested tuple: the law recurses to the leaf (`result.0.0`), not just `result.0`.
// Regression test for the structural recursion in `structural_clone_post`.
