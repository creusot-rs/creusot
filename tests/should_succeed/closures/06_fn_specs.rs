...
#[requires(f.precondition(a, mode!()))]
#[ensures(f.postcondition(a, result, mode!()))]
pub fn weaken_std<A: Tuple, F: Fn<A>>(f: F, a: A) -> F::Output {
    weaken_2_std(f, a)
}

#[requires(f.precondition(a, mode!()))]
#[ensures(exists<f2: F> f.postcondition_mut(a, f2, result, mode!()) && resolve(f2))]
fn weaken_2_std<A: Tuple, F: FnMut<A>>(f: F, a: A) -> F::Output {
    weaken_3_std(f, a)
}

#[requires(f.precondition(a, mode!()))]
#[ensures(f.postcondition_once(a, result, mode!()))]
...
#[requires(f.precondition((0usize,), mode!()))]
