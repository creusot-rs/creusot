...
#[requires(|mode| f.precondition(mode, a))]
#[ensures(|result, mode| f.postcondition(mode, a, result))]
pub fn weaken_std<A: Tuple, F: Fn<A>>(f: F, a: A) -> F::Output {
    weaken_2_std(f, a)
}

#[requires(|mode| f.precondition(mode, a))]
#[ensures(|result, mode| exists<f2: F> f.postcondition_mut(mode, a, f2, result) && resolve(f2))]
fn weaken_2_std<A: Tuple, F: FnMut<A>>(f: F, a: A) -> F::Output {
    weaken_3_std(f, a)
}

#[requires(|mode| f.precondition(mode, a))]
#[ensures(|result, mode| f.postcondition_once(mode, a, result))]
...
#[requires(|mode| f.precondition(mode, (0usize,)))]
