...
pub mod implementation {
    ...
    impl<T> LocalInvariantSpec for CompleteMap<T> {
        ...
        fn invariant_with_data(self, id: Id) -> bool {
            pearlite! {
                self.values.id() == id &&
                (forall<id: _> self.own_map.contains(id) == self.values@.contains(*id)) &&
                (forall<id: _> self.own_map.contains(id) ==> self.own_map[id].id() == *id) &&
                (forall<id: _> self.own_map.contains(id) ==> self.values@.lookup(*id).len() == *self.length) &&
                // If `Link { next, .. }` is in the map, then `next` is also in the map.
                (forall<id: _> self.own_map.contains(id) ==> match *self.own_map[id].val() {
                    Inner::Direct(_) => true,
    ...
    impl<T: 'static> PersistentArray<T> {
        ...
        pub fn set(&self, index: usize, value: T, namespaces: Ghost<Namespaces>) -> Self {
            ...
            let new_frac = {
                ...
                LocalInvariant::open(self.map_invariant, namespaces, |mut tokens| {
        ...
        pub unsafe fn get_immut<'a>(
            ...
            LocalInvariant::open(self.map_invariant, namespaces, |tokens| {
                // prove that self is contained in the map by validity
                ghost! {
                    tokens.values.contains(*self.contained_in_token);
                };
        ...
        #[ensures(forall<id: _> (*tokens).own_map.contains(id) == (^tokens.inner_logic()).own_map.contains(id))]
