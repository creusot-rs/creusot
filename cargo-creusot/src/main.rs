...
/// How to get proof.json from which to generate why3session.xml
#[derive(Debug, Clone, ValueEnum)]
pub enum WithProof {
    /// Run proof search to regenerate proof.json
    Prove,
...
fn why3(args: Why3Args) -> Result<()> {
    ...
    let paths = setup::creusot_paths()?;
    if let Some(with_proof) = args.with_proof {
        let replay = match with_proof {
