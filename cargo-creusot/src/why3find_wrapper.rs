...
// - `ide_always` is implemented in an ad hoc way here.
...
pub fn why3find_prove(mut args: ProveArgs, root: &PathBuf) -> Result<()> {
    ...
    let coma = if args.ide.ide_always {
        if args.files.len() == 1 && args.files[0].extension() == Some(OsStr::new("coma")) {
            Some(args.files[0].clone())
        } else {
            return Err(anyhow!("--ide-always requires exacly one Coma file argument"));
        }
    } else {
        None
    };
