...
impl SetupTool {
    fn managed(self) -> SetupManagedTool {
        use SetupManagedTool as M;
        use SetupTool::*;
        match self {
            Why3 | Why3find => M::Why3AndWhy3find,
            AltErgo => M::AltErgo,
            Z3 => M::Z3,
            CVC4 => M::CVC4,
            CVC5 => M::CVC5,
...
fn install_tools(paths: &setup::CreusotPaths, args: Args) -> anyhow::Result<()> {
    ...
    for (bin, tool) in [
        ...
        if args.external.contains(&tool.managed()) {
