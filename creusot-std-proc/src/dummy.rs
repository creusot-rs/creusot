...
pub fn snapshot(_: TS1) -> TS1 {
    quote::quote! { ::creusot_std::snapshot::Snapshot::from_fn(|| panic!() ) }.into()
}
...
/// Visitor to delete all `#[invariant]` and `#[creusot_std::invariant]`
/// attributes on loops.
...
fn is_qualified(name: &str, path: &syn::Path) -> bool {
    ...
    {
        return first.ident == "creusot_std" && second.ident == name;
    }
    false
}
