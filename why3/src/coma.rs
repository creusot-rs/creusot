...
impl Defn {
    pub fn simple(name: Ident, body: Expr) -> Self {
        Defn {
            prototype: Prototype { name: name, attrs: vec![], params: Box::new([]) },
            body,
        }
    }
}
    ...
    A::Doc: Clone,
{
    ...
    let doc = docs![
        ...
        alloc.intersperse(defn.prototype.attrs.iter().map(|a| a.pretty(alloc, scope)), alloc.space()),
        alloc.space(),
    ...
    A::Doc: Clone,
{
    ...
    let doc = NonRecDefnDoc {
        ...
        attrs: alloc.intersperse(defn.prototype.attrs.iter().map(|a| a.pretty(alloc, scope)), alloc.space()),
        params: alloc.intersperse(defn.prototype.params.iter().map(|a| a.pretty(alloc, scope)), " "),
