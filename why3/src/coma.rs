        ...
        A::Doc: Clone,
    {
        match self {
            ...
            Expr::Defn(cont, rec, handlers) => {
                scope.open();
                let doc = if *rec {
                    handlers.iter().for_each(|defn| scope.bind_value(defn.name));
    ...
    A::Doc: Clone,
{
    ...
    let doc = NonRecDefnDoc {
        name: defn.name,
        attrs: alloc.intersperse(defn.attrs.iter().map(|a| a.pretty(alloc, scope)), alloc.space()),
        params: alloc.intersperse(defn.params.iter().map(|a| a.pretty(alloc, scope)), " "),
