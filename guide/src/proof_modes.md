...
named `mode!()`. The status of each mode can be accessed using a logical method.
...
Modes now also appear as an argument to `precondition` and `postcondition`:

```rust
#[requires(f.precondition((), mode!()))]
#[ensures(f.postcondition((), result, mode!()))]
fn call(f: Fn() -> usize) -> usize {
    f()
}
```
...
```rust
#[check(terminates)]
#[requires(mode!().terminates() ==> 0isize <= x)]
#[requires(isize::MIN < x)]
#[ensures(0isize <= x)]
#[variant(x)]
fn f(x: isize) {
    if x != 0 {
...
- if the function is recursive, it must have a `#[variant]`
  and only call other terminating functions with a smaller variant,
...
For those functions, Creusot requires that their preconditions are syntactically of the form
`mode!().nopanic() ==> _`, otherwise a warning is emitted.

For example, an `index` function implemented using an unsafe `unsafe_index` function
would have the following precondition:

```rust
#[requires(mode!().nopanic() ==> i@ < x@.len())]
fn index<T>(x: &[T], i: usize) -> &T {
    unsafe {
        unsafe_index(x, i)
    }
}
...
Ghost code must terminate normally, thus `ghost` mode enables `nopanic` and `terminates`,
...
<!--
TODO: new attribute for "ghost-only" functions.
...
```rust
#[requires(f.precondition((x,), mode!()))]
#[ensures(f.postcondition((x,), result, mode!()))]
...
then we should check its precondition in ghost mode, denoted `mode!().into_ghost()`.

```rust
#[requires(f.precondition((x,), mode!().into_ghost()))]
#[ensures(f.postcondition((x,), result, mode!().into_ghost()))]
fn apply_ghost<A, B>(f: FnGhost(A) -> B, x: Ghost<A>) -> Ghost<B> {
    ghost! { f(x.into_inner()) }
}
```

Similarly, trait methods can carry pre- and postconditions depending on proof modes.
...
#[check(ghost)]
#[requires(<I as Iterator>::next.precondition((i,), result, mode!()))]
