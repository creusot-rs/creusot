...
Modes now also appear in the pre- and postconditions of closures:

```rust
#[requires(|mode| f.precondition((), mode))]
#[ensures(|result, mode| f.postcondition((), result, mode))]
...
```rust
#[requires(|mode| f.precondition((x,), mode))]
#[ensures(|result, mode| f.postcondition((x,), result, mode))]
...
```rust
#[requires(|mode| f.precondition((x,), mode.into_ghost()))]
#[ensures(|result, mode| f.postcondition((x,), result, mode.into_ghost()))]
...
#[check(ghost)]
#[requires(|mode| <I as Iterator>::next.precondition((i,), result, mode))]
