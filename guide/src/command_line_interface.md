...
- `--ide-always`: Open the Why3 IDE (requires `<COMA_FILE>`).
- `--replay`: Don't generate new proofs, only check if the existing proofs are valid.
- `--why3session`: Generate `why3session.xml` files (this may be useful with the command `cargo creusot why3 ide`; this is implied by `-i` and `--ide-always` so you don't need to know about this option in normal usage).
