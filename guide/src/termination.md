...
- Termination of ghost program functions is also necessary for soundness.
  ...
  and that would be difficult to guarantee if ghost code did not terminate.
...
- Termination of normal functions (non-ghost program functions) is optional, though desirable.
  ...
  and sometimes significantly harder to prove than partial correctness.
...
- a single item with no edge to itself, that is a non-recursive item which
  requires no further check: it is well-defined if all of its dependencies are well-defined;
...
#### Counterexample {#recursive-type-counterexample}
...
`Mapping<Dummy::A, bool>`, which would be unsound as explained in a [previous counterexample](#recursive-type-counterexample).
