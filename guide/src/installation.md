...
- `cargo`
- `opam` to install `why3` and `why3find`.
...
(usually `~/.local/share/creusot/_opam`). We recommend having this local
switch to prevent accidentally breaking your Creusot setup while working
on other OCaml projects.

## Manual installation

1. Install the core files from the Creusot repository:
...
2. Install binaries in `$XDG_DATA_HOME/creusot`:
...
- `$XDG_DATA_HOME/creusot/` (usually `~/.local/share/creusot`), a directory containing:
...
- The Creusot prelude, a Why3find package
    (with the local switch from the Quick installation,
...
(usually `~/.config/creusot`). This file is generated at the first invocation
