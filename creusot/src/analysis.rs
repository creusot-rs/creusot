...
use rustc_hir::{def_id::{DefId, LocalDefId}, HirId};
use rustc_index::{bit_set::MixedBitSet, Idx as _};
use rustc_macros::{TyDecodable, TyEncodable};
use rustc_middle::{mir::{self, traversal, BasicBlock, Local, Location, Place}, ty::{self, TyCtxt, TypingEnv}};
use rustc_mir_dataflow::{impls::MaybeUninitializedPlaces, move_paths::{HasMoveData, LookupResult, MoveData, MovePathIndex}, on_all_children_bits, Analysis as _, ResultsCursor};
use rustc_serialize::{Decodable, Decoder, Encodable, Encoder};
use rustc_span::Symbol;
use rustc_type_ir::TyKind;
use why3::Ident;

use crate::{analysis::resolve::{place_contains_borrow_deref, HasMoveDataExt as _, Resolver}, contracts_items::{is_snapshot_closure, is_spec}, ctx::{body_with_facts, HasTyCtxt, TranslationCtx}, extended_location::ExtendedLocation, gather_spec_closures::{corrected_invariant_names_and_locations, LoopSpecKind, SpecClosures}, naming::variable_name, translation::{fmir::{self, BorrowKind}, pearlite::{self, Term}}};
...
impl<'tcx> BodyData<'tcx> {
    ...
    pub(crate) fn resolved_places_at(&self, loc: Location) -> &[Place<'tcx>] {
        self.resolved_at.get(&Orphan(loc)).map_or(&[], |v| &v[..])
    }
...
impl<'a, 'tcx> Analysis<'a, 'tcx> {
        ...
        for mp in to_resolve_partial.iter() {
            ...
            match ty.ty.kind() {
                ...
                Array(_, _) | Slice(_) | Pat(_, _) => {
                    self.span_bug(local_decls[pl.local].source_info.span, format!(
    ...
    fn resolve_places_between_blocks(&mut self, bb: BasicBlock) {
        ...
        fn discriminator_for_switch<'tcx>(bbd: &mir::BasicBlockData<'tcx>) -> Option<Place<'tcx>> {
            let discr = if let mir::TerminatorKind::SwitchInt { discr, .. } = &bbd.terminator().kind {
                discr
            } else {
                return None;
            };
            if let mir::StatementKind::Assign(box (pl, mir::Rvalue::Discriminant(real_discr))) =
    ...
    fn run(&mut self) {
        for (bb, bbd) in traversal::reverse_postorder(self.resolver.body) {
            ...
            let tcx = self.tcx();
            if let Some(invariants) = self.body_specs.invariants.get_mut(&bb)
                && !invariants.is_empty()
            {
    ...
    fn analyze_terminator(&mut self, terminator: &mir::Terminator<'tcx>, mut loc: Location) {
        ...
        match terminator.kind {
            ...
            Call { ref func, destination, target, fn_span, .. } => {
                ...
                // (if this is not a snapshot, it won't be in `self.body_specs.snapshots` so this will be a noop)
                let Some((_fun_def_id, subst)) = func_defid(func) else {
    ...
    fn check_final(&mut self, pl: &Place<'tcx>, loc: Location) {
        let is_final =
            NotFinalPlaces::is_final_at(&mut self.not_final_places, pl, loc.successor_within_block());
