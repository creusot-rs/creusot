...
pub(crate) fn translate_ty<'tcx, N: Namer<'tcx>>(
    ...
    match ty.kind() {
        Bool => MlT::qconstructor("bool".into()),
        ...
        Array(ty, _) => MlT::qconstructor(names.in_pre(PreMod::Slice, "array"))
            .tapp([translate_ty(ctx, names, span, *ty)]),
        Str => MlT::qconstructor("string".into()),
        Never => MlT::unit(),
...
pub(crate) fn eliminator<'tcx, N: Namer<'tcx>>(
    ...
    let ret_ident = Ident::fresh("ret");
    let good_ident = Ident::fresh("good");
    let bad_ident = Ident::fresh("bad");
    let input_ident = Ident::fresh("input");
