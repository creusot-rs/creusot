...
use rustc_hir::{def_id::DefId, def::DefKind};
...
use rustc_middle::ty::{DefIdTree, TyKind, Ty, InternalSubsts, FieldDef, SubstsRef, TyCtxt, self};
...
fn translate_ty_inner<'tcx, C: Cloner<'tcx>>(
    ...
    match ty.kind() {
        ...
        Ref(_, ty, borkind) => {
            match borkind {
                Mutability::Mut => MlT::MutableBorrow(box translate_ty_inner(trans, ctx, names, span, *ty)),
                Mutability::Not => translate_ty_inner(trans, ctx, names, span, *ty),
            }
        }
        Slice(ty) => MlT::TApp(
...
pub(crate) fn translate_tydecl<'tcx, C: Cloner<'tcx>>(
    ...
    let name = module_name(ctx.tcx, did);
...
pub(crate) fn lower_accessor<'tcx>(
    ...
    vec![stub, Module { name: module_name(ctx.tcx, def_id), decls }]
}
...
pub(crate) fn build_accessor<'tcx>(
    ...
    for (ix, v) in def.variants().iter_enumerated() {
        ...
        let p =
            pearlite::Pattern::Constructor { adt: def.did(), substs, variant: ix, fields: pats };
        arms.push((p, exp))
    }
...
pub(crate) fn build_closure_accessor<'tcx>(
    ...
    let term = Term {
        ty: out_ty,
        kind: TermKind::Let {
            pattern: Pattern::Constructor { adt: closure, substs, variant: 0u32.into(), fields },
            arg: box self_,
            body: box res,
        },
        span: DUMMY_SP,
...
pub(crate) fn intty_to_ty(ity: &rustc_middle::ty::IntTy) -> MlT {
...
pub(crate) fn uintty_to_ty(ity: &rustc_middle::ty::UintTy) -> MlT {
...
pub(crate) fn floatty_to_ty(fty: &rustc_middle::ty::FloatTy) -> MlT {
