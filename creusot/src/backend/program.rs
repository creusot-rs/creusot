...
pub fn val<'tcx>(_: &Why3Generator<'tcx>, sig: Signature) -> Decl {
    ...
    let params = sig
        ...
        .chain([Param::Cont(
            ret_ident,
            Vec::new(),
            vec![Param::Term(result_ident, sig.retty.clone().unwrap())],
        )])
        .collect();
    ...
    let mut postcond = Expr::Variable(ret_ident).app(vec![Arg::Term(Exp::Var(result_ident))]);
    ...
    let body = Expr::Defn(
        ...
        vec![Defn {
            ...
            writes: Vec::new(),
            attrs: vec![],
            params: vec![Param::Term(result_ident, sig.retty.clone().unwrap())],
            body: postcond,
        }],
    );
...
pub fn to_why<'tcx, N: Namer<'tcx>>(
    ...
    let params = sig
        ...
        .chain([Param::Cont(
            ret_ident,
            Vec::new(),
            vec![Param::Term(result_ident, sig.retty.unwrap())],
        )])
        .collect();
    ...
    pub(crate) fn to_why<N: Namer<'tcx>>(
        ...
        let e = match self {
            ...
            RValue::BinOp(op, l, r) => {
                ...
                istmts.extend([IntermediateStmt::call(ret, lower.ty(ty), call, args)]);
            ...
            RValue::Cast(e, source, target) => {
                match source.kind() {
                    TyKind::Bool => {
                        ...
                        let of_ret_id: Ident = Ident::fresh("_ret_from");
                    ...
                    _ => {
                        ...
                        let to_ret_id: Ident = Ident::fresh("_ret_to");
            ...
            RValue::Array(fields) => {
                ...
                let mut assumptions = fields
                    ...
                    .chain([Exp::qvar(QName::from("Seq.length"))
            ...
            RValue::Repeat(e, len) => {
                ...
                istmts.push(IntermediateStmt::call(
                    res.clone(),
                    lower.ty(ty),
                    Expr::Constant(lower.names.from_prelude(PreludeModule::Slice, "create")),
                    args,
                ));
            ...
            RValue::Ptr(pl) => {
                ...
                istmts.push(IntermediateStmt::call(
                    ptr.clone(),
                    lower.ty(ty),
                    Expr::Constant("Opaque.fresh_ptr".into()),
                    vec![],
                ));
...
impl<'tcx> Terminator<'tcx> {
    ...
    pub(crate) fn to_why<N: Namer<'tcx>>(
        ...
        match self {
            ...
            Terminator::Return => {
                (istmts, Expr::Variable(Ident::bound("ret")).app(vec![Arg::Term(Exp::Var(Ident::fresh("_0")))])) // TODO where is _0 bound
            }
...
fn mk_goto(bb: BasicBlock) -> coma::Expr {
    coma::Expr::Variable(Ident::fresh(format!("bb{}", bb.as_u32()))) // TODO
}
...
fn mk_switch_branches(discr: Exp, brch: Vec<(Exp, coma::Expr)>) -> Vec<coma::Defn> {
    ...
    for (ix, (cond, tgt)) in brch.into_iter().enumerate() {
        ...
        let branch = coma::Defn::simple(Ident::fresh(format!("br{ix}")), filter); // TODO
        out.push(branch)
    }
    out
}
    ...
    pub(crate) fn to_why<N: Namer<'tcx>>(
        ...
        match self {
            Statement::Assignment(lhs, RValue::Borrow(bor_kind, rhs, triv_inv), _span) => {
                ...
                let func = coma::Expr::Constant(lower.names.from_prelude(
            ...
            Statement::Call(dest, fun_id, subst, args, _) => {
                ...
                let ret_ident = Ident::fresh("_ret'");
                istmts.push(IntermediateStmt::call(ret_ident, ty, fun_exp, args));
