...
pub fn to_why<'tcx, N: Namer<'tcx>>(
    ...
    let blocks: Box<[Defn]> = wto
        .into_iter()
        .map(|c| component_to_defn(&mut body, ctx, names, body_id.def_id, c))  // TODO
        .collect();
        ...
        istmts: &mut Vec<IntermediateStmt>,
         ...
         match self {
                    ...
                    BinOp::Shl | BinOp::ShlUnchecked | BinOp::Shr | BinOp::ShrUnchecked => {
                        ...
                        let qname = match r_ty.kind() {
                            TyKind::Int(ity) => lower
                                .names
                                .from_prelude(ity_to_prelude(lower.ctx.tcx, *ity), "to_int"),
                            TyKind::Uint(uty) => lower
                                .names
                                .from_prelude(uty_to_prelude(lower.ctx.tcx, *uty), "t'int"),
                            _ => unreachable!(
            ...
            RValue::UnaryOp(UnOp::Neg, arg) => {
                ...
                let ret: Ident = Ident::fresh("_ret");
                let arg = Arg::Term(arg.to_why(lower, istmts));
                istmts.push(IntermediateStmt::call(ret, lower.ty(ty), neg, Box::new([arg])));
                Exp::Var(ret)
            }
...
impl<'tcx> Block<'tcx> {
    pub(crate) fn to_why<N: Namer<'tcx>>(
        ...
        for (ix, s) in self.stmts.into_iter().enumerate() {
            ...
            statements.push(Defn::simple(Ident::fresh(format!("s{}", ix)), body));  // TODO
        }

        let body = assemble_intermediates(istmts.into_iter(), terminator);
        statements.push(Defn::simple(Ident::fresh(format!("s{}", statements.len())), body)); // TODO
        ...
        Defn::simple(Ident::fresh(format!("bb{}", id.as_usize())), body) // TODO
    }
}
    ...
    I: DoubleEndedIterator<Item = IntermediateStmt>,
{
    istmts.rfold(exp, |tail, stmt| match stmt {
        ...
        IntermediateStmt::Call(params, fun, args) => {
            ...
            args.into_iter()
                .chain([Arg::Cont(c)])
                .fold(Expr::Constant(fun), |acc, arg| Expr::App(Box::new(acc), Box::new(arg)))
        }
    ...
    pub(crate) fn to_why<N: Namer<'tcx>>(
        ...
        match self {
            Statement::Assignment(lhs, RValue::Borrow(bor_kind, rhs, triv_inv), _span) => {
                ...
                let func = lower.names.from_prelude(PreludeModule::MutBorrow, match bor_kind {
                ...
                if let BorrowKind::Final(deref_index) = bor_kind {
                    let (original_borrow_ty, original_borrow, original_borrow_constr) =
                        projections_to_expr(
                            ...
                            &mut istmts,
                            rhs_local_ty,
                            Focus::new(|_| Exp::Var(lower.ctx.ident(rhs.local))),
                            Box::new(|_, x| x),
                    ...
                    let borrow_id = borrow_generated_id(
                        ...
                        |ident| {
                            ...
                            .app([Exp::Var(lower.ctx.ident(*ident))])
                        },
                    );
                    ...
                    let (_, foc, constr) = projections_to_expr(
                        ...
                        &mut istmts,
                        rhs_local_ty,
                        Focus::new(|_| Exp::Var(lower.ctx.ident(rhs.local))), // TODO
                        Box::new(|_, x| x),
                        &rhs.projection,
                    );
                    rhs_rplace = foc.call(&mut istmts);
...
fn exp_of_place<'tcx, N: Namer<'tcx>>(lower: &LoweringState<'_, 'tcx, N>, rp: Exp, pl: Place<'tcx>) -> Exp {
    ...
    let local = lower.ctx.ident(local);
    if let WPattern::VarP(_) = pat {
        rp.app([Exp::Var(local)])
    } else {
        Exp::Match(
            Box::new(Exp::Var(local)),
            Box::new([
                (pat, rp.app([Exp::Var(local)])), // TODO the variable name looks wrong
