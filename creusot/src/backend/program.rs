...
use crate::{
    backend::{
        ...
        term::{cast_int, lower_pure, unsupported_cast},
        ty::{
...
impl<'tcx> RValue<'tcx> {
    ...
    fn into_why(
        ...
        match self {
            RValue::Operand(op) => op.into_why(lower, istmts, span),
            RValue::BinOp(op, l, r) => {
                ...
                match (op, l_ty.kind()) {
                    (_, TyKind::Float(_)) => (),
                    (BinOp::Eq, _) => {
                        return l.into_why(lower, istmts, span).eq(r.into_why(lower, istmts, span));
                    }
                    (BinOp::Ne, _) => {
                        return l
                            .into_why(lower, istmts, span)
                            .neq(r.into_why(lower, istmts, span));
                    }
                    _ => (),
                }

                let prelude = ty_to_prelude(lower.ctx.tcx, l_ty);
                ...
                let r = match op {
                    ...
                    BinOp::Shl | BinOp::ShlUnchecked | BinOp::Shr | BinOp::ShrUnchecked => {
                        ...
                        // so we need to cast the right operand
                        let conv = if lower.names.bitwise_mode() {
                            "to_BV256"
                        } else if let TyKind::Int(_) = r_ty.kind() {
                            "to_int"
                        } else {
                            "t'int"
                        };
                        let qname = lower.names.in_pre(ty_to_prelude(lower.ctx.tcx, r_ty), conv);

                        // build the expression for this convertion
                        Exp::qvar(qname).app([r.into_why(lower, istmts, span)])
                    }
                    _ => r.into_why(lower, istmts, span),
                };
                ...
                let args = [l.into_why(lower, istmts, span), r];
            ...
            RValue::UnaryOp(UnOp::Not, arg) => {
                ...
                match a_ty.kind() {
                    TyKind::Bool => arg.into_why(lower, istmts, span).not(),
            ...
            RValue::Constructor(id, subst, args) => {
                ...
                let args = args.into_iter().map(|a| a.into_why(lower, istmts, span)).collect();
                constructor(lower.names, args, id, subst)
            }
            RValue::Tuple(flds) if flds.is_empty() => Exp::unit(),
            RValue::Tuple(flds) if flds.len() == 1 => {
                flds.into_iter().next().unwrap().into_why(lower, istmts, span)
            }
            RValue::Tuple(flds) => {
                let TyKind::Tuple(tys) = ty.kind() else { unreachable!() };
                Exp::Record {
                    fields: flds
                        ...
                        .map(|(ix, f)| {
                            (
                                lower.names.tuple_field(tys, ix.into()),
                    ...
                    && let TyKind::Dynamic(_, _) = target.kind() =>
            {
                ...
                Exp::var(cast).app([e.into_why(lower, istmts, span)])
            }
            RValue::Cast(e, source, target) => {
                let arg = e.into_why(lower, istmts, span);
                match source.kind() {
                    TyKind::Bool => {
                    ...
                    TyKind::Int(_) | TyKind::Uint(_) => match target.kind() {
                        TyKind::Char => {
                            // convert source to BV256.t / int
                            let to_fname = if lower.names.bitwise_mode() {
                                "to_BV256"
                            } else if let TyKind::Int(_) = source.kind() {
                                "to_int"
                            } else {
                                "t'int"
                            };
                            let to_exp = Exp::qvar(
                                lower.names.in_pre(ty_to_prelude(lower.ctx.tcx, source), to_fname),
                            )
                            .app([arg]);
                        ...
                        TyKind::Int(_) | TyKind::Uint(_) => {
                            cast_int(lower.names, source, target, arg)
                        }
                        _ => unsupported_cast(lower.ctx, span, source, target),
                    },
            ...
            RValue::Repeat(e, len) => {
                let args = [
                    ...
                    Arg::Term(len.into_why(lower, istmts, span)),
                    Arg::Term(Exp::Lam(
                        [Binder::wild(int(lower.ctx, lower.names))].into(),
                        e.into_why(lower, istmts, span).boxed(),
                    )),
                ];
            ...
            RValue::UnaryOp(UnOp::PtrMetadata, op) => {
                match op.ty(lower.ctx.tcx, lower.locals).kind() {
                    TyKind::Ref(_, ty, mu) => {
                        assert!(ty.is_slice());
                        let mut op = op.into_why(lower, istmts, span);
                        if mu.is_mut() {
...
impl<'tcx> Terminator<'tcx> {
    fn into_why(
        ...
        let exp = match self {
            ...
            Terminator::Switch(switch, span, branches) => {
                let ty = switch.ty(lower.ctx.tcx, lower.locals);
                let discr = switch.into_why(lower, &mut istmts, span);
...
fn func_call_to_why3<'tcx>(
    ...
    let args: Box<[_]> = if lower.ctx.is_closure_like(id) {
        ...
        args.into_iter()
            .map(|a| a.into_why(lower, istmts, span))
            .map(Arg::Term)
            .chain(once(mode))
            .collect()
    };
