...
use crate::{
    ...
    naming::STATIC_NAMES,
    pearlite::{Term, TermKind, normalize},
...
/// Substitution with a very small array
fn array_subst<'a>(subst: &[(Ident, Ident)]) -> impl Fn(Ident) -> Option<TermKind<'a>> {
    move |x| {
...
fn fn_once_postcond_term<'tcx>(
    ...
    let isubst =
        &[(STATIC_NAMES.self_, self_), (STATIC_NAMES.args, args), (STATIC_NAMES.result, result)];
    let tcx = ctx.tcx;
    ...
    match ty_self.kind() {
        TyKind::Closure(did, _) => {
            ...
            term.iter_mut().for_each(|term| term.subst_with(array_subst(isubst)));
            term
        }
...
fn fn_mut_postcond_term<'tcx>(
    ...
    let isubst = &[
        (STATIC_NAMES.self_, self_),
        (STATIC_NAMES.args, args),
        (STATIC_NAMES.result_state, result_state),
        (STATIC_NAMES.result, result),
    ];
    let tcx = ctx.tcx;
    let ty_self = subst.type_at(1);
    let self_ = Term::var(self_, ty_self);
...
fn fn_mut_unnest_term<'tcx>(
    ...
    let isubst = &[(STATIC_NAMES.self_, self_), (STATIC_NAMES.future, future)];
    let TyKind::Closure(did, _) = subst.type_at(1).kind() else { return None };
