...
impl<'tcx> Lower<'_, 'tcx> {
            ...
            TermKind::Call {
                ...
                self.lookup_builtin(method, &mut args).unwrap_or_else(|| {
                    ...
                    if self.pure == Purity::Program {
                        mk_binders(Exp::QVar(self.names.get(method), self.pure), args)
                    } else {
                        Exp::Call(box Exp::QVar(self.names.get(method), self.pure), args)
                    }
                })
            }
    ...
    fn lower_ty(&mut self, ty: Ty<'tcx>) -> Type {
        super::ty::translate_ty(self.ctx, self.names, creusot_rustc::span::DUMMY_SP, ty)
    }
}
...
pub(crate) fn lower_literal<'tcx>(
    ...
    match lit {
        Literal::Int(u, intty) => {
            let why_ty = intty_to_ty(ctx, &intty);

            Constant::Int(u, Some(why_ty)).into()
        }
        Literal::Uint(u, uty) => {
            let why_ty = uintty_to_ty(ctx, &uty);
