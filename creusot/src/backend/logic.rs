...
pub(crate) fn translate_logic_or_predicate<'tcx>(
    ...
    // Also need to collect dependencies from the signature
    let graph = collect(ctx, def_id);
    ...
    if util::is_trusted(ctx.tcx, def_id) || !util::has_body(ctx, def_id) {
        ...
        let body = term::lower_pure(ctx, &names, ctx.param_env(def_id), term);
    ...
    (stub_module(ctx, def_id), Module { name, decls }, proof_modl, has_axioms)
}
