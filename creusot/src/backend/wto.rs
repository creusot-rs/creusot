...
pub fn reconstruct<G>(g: G, root: G::NodeId) -> Vec<Component<G::NodeId>>
where
    G: IntoNeighborsDirected + Visitable,
    <G as GraphBase>::NodeId: Eq + Hash,
{
    // Find the loop structure
    let wto = weak_topological_order(g, root);
    let dom = petgraph::algo::dominators::simple_fast(g, root);

    // let mut component = Vec::new();

    // // Fold over the loops using dominance information to push sequences as far deep as possible.
    // for c in wto.into_iter().rev() {
    //     match c {
    //         Component::Vertex(v) => component.push(v),
    //         Component::Component(hd, body) => {

    //         },
    //         // Component::Sequence(_) => unreachable!(),
    //     }
    // }
