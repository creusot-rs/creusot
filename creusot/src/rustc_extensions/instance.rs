...
) -> Result<Option<Instance<'tcx>>, DiagnosticBuilder<'tcx>> {
    let (param_env, (did, substs)) = key.into_parts();
    if let Some(did) = did.as_local() {
        if let Some(param_did) = tcx.opt_const_param_of(did) {
            return resolve_instance_of_const_arg(tcx, param_env.and((did, param_did, substs)));
        }
    }
...
fn inner_resolve_instance<'tcx>(
    ...
    let result = if let Some(trait_def_id) = tcx.trait_of_item(def.did) {
        // panic!(" => associated item, attempting to find impl in param_env {:#?}", param_env);
    ...
    // panic!("inner_resolve_instance: result={:?}", result);
    result
}
    ...
    Ok(match vtbl {
        traits::ImplSource::UserDefined(impl_data) => {
            ...
            let leaf_def = trait_def
                ...
                .unwrap_or_else(|| {
                    panic!("{:?} not found in {:?}", trait_item, impl_data.impl_def_id);
                });
                ...
                && leaf_def.item.def_id.is_local()
            {
                ...
                if original_ty != resolved_ty {
                    ...
                    return Err(tcx.sess.struct_span_err(span, &msg));
                }
            }
