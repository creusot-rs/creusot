...
impl ForbidsRecursion {
    fn display(self, ctx: &TranslationCtx) -> String {
        ...
        match self {
            Opaque(def_id) => format!("opaque type `{}`", ctx.def_path_str(def_id)),
    ...
    match graph.graph[node] {
        ...
        Type(type_id) => walk_adt(ctx, type_id, |tgt, path, span| {
            ...
            if scc.contains(&index) {
                use TypeNode::*;
                match tgt {
                    Type(_) => forbid_recursion(ctx, positivity, path),
...
fn forbid_recursion<'tcx>(
    ...
    path.iter().try_for_each(|(ty, index)| match ty.kind() {
        &Adt(def, subst) => {
            ...
            match classify_adt(ctx, def.did(), def, subst) {
                Opaque { .. } | Builtin(_) | Identity(_) => Err(Opaque(def.did())),
                Snapshot(_) | Box(_) => Ok(()),
                Enum | Struct { .. } => {
                ...
                Unit | Empty | Namespace => unreachable!(),
            }
        }
...
fn get_positivity(ctx: &TranslationCtx, def_id: DefId) -> DenseBitSet<usize> {
    ...
    for field in ctx.adt_def(def_id).all_fields() {
            ...
            match ty.kind() {
                &Adt(def, args)
                    if let K::Box(arg) | K::Snapshot(arg) =
                        classify_adt(ctx, def.did(), def, args) =>
                {
                    ty = arg
                }
                &Ref(_, arg, _) => ty = arg,
...
/// Only used for emitting error messages.
#[derive(Debug)]
enum TypeEdge {
    AdtDef(Span),
...
/// Add dependencies on traits that appear in bounds of the trait `def_id` itself and on its methods.
/// We do not need to track dependencies between traits and types:
/// we can decompose a trait into (1) its associated types, which are basically extra type parameters,
/// and (2) its methods, which belong to a different "phase" from types and traits (in the sense
/// of "the phase distinction".) Functions depends on types, but types never depend on functions,
/// so there are no cycles intersecting with both phases.)
fn add_trait(ctx: &TranslationCtx, def_id: DefId, graph: &mut TypeGraph) {
    if is_trusted_item(ctx.tcx, ctx.parent(def_id)) {
        return;
    }
