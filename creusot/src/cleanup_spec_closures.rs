...
thread_local! {
    ...
    pub static LOGICAL_STUFF:
        RefCell<HashMap<LocalDefId, LogicalStuff<'static>>> =
        RefCell::new(HashMap::new());
}
...
pub(crate) fn cleanup_spec_closures<'tcx>(tcx: TyCtxt<'tcx>, def_id: DefId, body: &mut Body<'tcx>) {
    ...
    if is_no_translate(tcx, def_id) || is_logic(tcx, def_id) {
        ...
        PEARLITE_MIR.with(|map| {
            ...
            map.borrow_mut().insert(local, bb);
        });
    } else {
        ...
        LOGICAL_STUFF.with(|map| {
            // SAFETY: Consumers cast the lifetime back to 'tcx
            let stuff = unsafe { std::mem::transmute(LogicalStuff { closures, assigns }) };
            map.borrow_mut().insert(local, stuff);
        });
    ...
    for data in body.basic_blocks_mut() {
        data.statements.retain(|statement| match &statement.kind {
            ...
            StatementKind::Assign(box (place, rvalue)) => {
                ...
                if dropped {
                    assigns.insert(*place, rvalue.clone());
                }
                !dropped
            }
