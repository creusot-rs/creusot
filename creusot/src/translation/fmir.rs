use super::{
    function::place::{self, translate_rplace_inner2},
...
impl<'tcx> Expr<'tcx> {
    ...
    pub(crate) fn to_why2(
        ...
        match self {
            Expr::Place(pl) => {
                translate_rplace_inner2(ctx, names, body.unwrap(), pl.local, pl.projection)
            }
            Expr::Move(pl) => {
                // TODO invalidate original place
                translate_rplace_inner2(ctx, names, body.unwrap(), pl.local, pl.projection)
            ...
            Expr::UnaryOp(op, arg) => {
                Exp::UnaryOp(unop_to_unop(op), box arg.to_why2(ctx, names, body))
            }
            Expr::Constructor(id, subst, args) => {
                let args = args.into_iter().map(|a| a.to_why2(ctx, names, body)).collect();
            ...
            Expr::Call(id, subst, args) => {
                let mut args: Vec<_> =
                    args.into_iter().map(|a| a.to_why2(ctx, names, body)).collect();
                // let fname = names.insert(id, subst).qname(ctx.tcx, id);
                let fname = names.get((id, subst));
            ...
            Expr::Constant(c) => backend::term::lower_literal(ctx, names, c),
            Expr::Tuple(f) => {
                Exp::Tuple(f.into_iter().map(|f| f.to_why2(ctx, names, body)).collect())
            }
            Expr::Span(sp, e) => {
                let e = e.to_why2(ctx, names, body);
                ctx.attach_span(sp, e)
            } // Expr::Cast(_, _) => todo!(),
            Expr::Cast(e, source, target) => {
                let to_int = match source.kind() {
                ...
                from_int.app_to(to_int.app_to(e.to_why2(ctx, names, body)))
            }
            Expr::Len(pl) => {
                let int_conversion = uint_from_int(&UintTy::Usize);
                let len_call = Exp::impure_qvar(QName::from_string("Seq.length").unwrap())
                    .app_to(pl.to_why2(ctx, names, body));
                int_conversion.app_to(len_call)
            }
        }
    }
