...
impl ContractClauses {
    ...
    fn get_pre<'tcx>(
        ...
        let bound_with_result =
            &bound.into_iter().chain(std::iter::once(STATIC_NAMES.result)).collect::<Box<_>>();
...
pub(crate) fn contract_of<'tcx>(ctx: &TranslationCtx<'tcx>, def_id: DefId) -> PreSignature<'tcx> {
    ...
    if let Some(spec) = ctx.extern_spec(def_id).cloned() {
        ...
        let raw_inputs = if inputs.len() > 0 && inputs[0].0.0 == STATIC_NAMES.self_ {
            &inputs[1..]
        } else {
            &inputs
        };
...
impl<'tcx> PreSignature<'tcx> {
    ...
    pub(crate) fn add_type_invariant_spec(
        ...
        let new_requires = self.inputs.iter().enumerate().filter_map(|(i, (ident, span, ty))| {
            ...
            {
                let expl = format!("expl:{} '{}' type invariant", fn_name, ident.0.name());
                ...
                .map(|(ix, param)| match &param.pat {
                    ...
                    None => {
                        if ix != 0 {
                            ctx.fatal_error(
                                span,
                                "only the first parameter is expected to be anonymous",
                            )
                            .emit();
                        }
                        // let env_ty = ctx.closure_env_ty(param.ty, subst.as_closure().kind(), tcx.lifetimes.re_erased);
                        (Ident::fresh(&format!("_{ix}")).into(), DUMMY_SP, param.ty.clone())
                    }
                })
                .collect();
...
pub fn inputs_and_output<'tcx>(
    ...
    match ty.kind() {
        ...
        TyKind::Closure(_, subst) => {
            ...
            let closure_env = (STATIC_NAMES.self_.into(), tcx.def_span(def_id), env_ty);
