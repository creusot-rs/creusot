...
/// `TypeVisitable`, `TypeFoldable`, `Decodable`, `Encodable` (derive macros `TyDecodable` and `TyEncodable`).
    ...
    let bound: Box<[(PIdent, Ty<'tcx>)]> = patterns
        ...
        .map(|(idx, pat)| {
            let ident = match pat.kind {
                ...
                _ => Ident::fresh(&format!("__{}", idx)).into(),
            };
            (
                ident,
...
impl<'tcx> Term<'tcx> {
    ...
    pub(crate) fn subst(&mut self, inv_subst: &std::collections::HashMap<Ident, TermKind<'tcx>>) {
        self.subst_with(|k| inv_subst.get(&k).cloned());
    }

    pub(crate) fn subst_with<F: FnMut(Ident) -> Option<TermKind<'tcx>>>(&mut self, mut f: F) {
        self.subst_with_inner(&HashSet::new(), &mut f)
    }

    fn subst_with_inner<F: FnMut(Ident) -> Option<TermKind<'tcx>>>(
        &mut self,
        bound: &HashSet<Ident>,
        inv_subst: &mut F,
    ) {
...
impl<'tcx> FTerm<'tcx> {
    ...
    pub fn instantiate(&self, idents: &[Ident]) -> Term<'tcx> {
