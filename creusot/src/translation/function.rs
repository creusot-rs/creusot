...
fn translate_vars<'tcx>(
    ...
    for (loc, d) in body.local_decls.iter_enumerated() {
        ...
        let ident = Ident::fresh(name.as_str());
...
impl<'tcx> TranslationCtx<'tcx> {
    pub(crate) fn build_closure_contract(&self, def_id: DefId) -> ClosureContract<'tcx> {
        ...
        let arg_tuple = Term::var(STATIC_NAMES.args, arg_ty);
        ...
        let self_ = Term::var(STATIC_NAMES.self_, env_ty);
        ...
        let postcond = |target_kind| {
            let postcondition = if contract.is_empty() {
                ...
                ret_params.push(Term::var(STATIC_NAMES.result, output));
        ...
        if kind.extends(ClosureKind::FnMut) {
            let self_ = Term::var(STATIC_NAMES.self_, env_ty);
            let result_state = Term::var(STATIC_NAMES.result_state, env_ty);
    ...
    fn inferred_precondition_term(
        ...
        let bor_self_ident = Ident::fresh("bor_self");
        ...
        match closure_kind {
            ...
            ClosureKind::FnMut => {
                ...
                if postcond_kind == ClosureKind::FnMut {
                    base = base.conj(
                        bor_self.fin().eq(self.tcx, Term::var(STATIC_NAMES.result_state, env_ty)),
                    )
                }
    ...
    for (ix, (capture, ty)) in captures.zip(subst.as_closure().upvar_tys()).enumerate() {
        match capture.info.capture_kind {
            ...
            UpvarCapture::ByRef(is_mut) => {
                ...
                let cur = self_.clone();
                let fin = Term::var(STATIC_NAMES.future, env_ty);
