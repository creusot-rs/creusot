...
impl<'body, 'tcx> BodyTranslator<'body, 'tcx> {
    ...
    fn with_context<R, F: for<'b> FnOnce(BodyTranslator<'b, 'tcx>) -> R>(
        ...
        let body = match body_id.constness {
            Constness::None | Constness::Const(_) => &body_with_facts.body,
            Constness::Promoted(promoted) => body_with_facts.promoted.get(promoted).unwrap(),
        };
